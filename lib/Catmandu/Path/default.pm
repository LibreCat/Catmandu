...
sub getter {
    ...
    my $body
        = $self->_emit_declare_vars($vals_var, '[]')
...
sub _emit_walk_path {
    ...
    if ($key =~ /^[0-9]+$/) {
        ...
        $perl .= $self->_emit_walk_path($var, [@$path], $cb);
        ...
        $perl .= $self->_emit_foreach(
            $var,
            sub {
                $self->_emit_walk_path(shift, $path, $cb);
            }
        );
        $perl .= "}";
    }
