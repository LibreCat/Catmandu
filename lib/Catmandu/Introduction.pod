...
Perl modules such as WebService::Solr or ElasticSearch provide easy access to your favorite document stores,
...
Create a Perl hash excute the function 'add' and your record is stored into the database.
Execute 'get' to load a Perl hash from the database in memory.
...
The Iterable package provides many more functions such as: to_array, count, each, first, slice,
take, group, tap, detect, select, reject, any, many, all, map, reduce and invoke.
