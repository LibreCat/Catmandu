...
#options/arguments
has path       => ( fix_arg => 1 );
has store_name => ( fix_arg => 1, default => sub { 'default' } );
...
search_in_store(path,store)

search_in_store(path,store, bag: 'bag', limit: 0, start: 0, sort: 'title desc')
...
e.g. fix:
    ...
    search_in_store('foo.query','catalog', bag: 'data', url: 'http://localhost:8983/solr/catalog')
...
=head1 EXAMPLES
    ...
    #search in Catmandu->store->bag, and store first 20 results in the foo.query.hits
    search_in_store('foo.query','default')

    #search in Catmandu->store->bag; limit number of results to 10
    search_in_store('foo.query','default', limit: 10)

    #search in Catmandu->store->bag; limit number of result to 10, starting from 15
    search_in_store('foo.query','default', limit: 10, start: 15)

    #search in Catmandu->store->bag; sort by year descending, and by title ascending
    search_in_store('foo.query','default', sort: 'year desc,title asc')
