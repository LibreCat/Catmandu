...
# Strings
is_deeply $cond->fix({foo => "true"}), {foo => "true", test => 'pass'};

is_deeply $cond->fix({foo => "false"}), {foo => "false", test => 'fail'};

# Boolean
my $hash = decode_json(qq|{"foo":true}|);
is_deeply $cond->fix($hash), {%$hash, test => 'pass'};

my $hash2 = decode_json(qq|{"foo":false}|);
is_deeply $cond->fix($hash2), {%$hash2, test => 'fail'};
...
is_deeply $cond->fix({foo => Cpanel::JSON::XS::true}),
    {foo => Cpanel::JSON::XS::true, test => 'pass'};

is_deeply $cond->fix({foo => Cpanel::JSON::XS::false}),
