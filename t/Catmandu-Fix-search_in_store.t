...
{
    my $got = $pkg->new('sessions', store => 'db')->fix({ sessions => 'njfranck' });
    my $expected = { sessions => { start => 0, limit => 20, total => 0, hits => [] } };
    is_deeply $got,$expected, "search in store db, bag data with query";
}

{
    my $got = $pkg->new('sessions', store => 'db', bag => 'sessions')->fix({ sessions => 'njfranck' });
...
{
    my $got = $pkg->new('sessions', store => 'db', bag => 'sessions', limit => 2)->fix({ sessions => '' });
    my $expected = {
        sessions => {
            start => 0,
            limit => 2,
...
{
    my $got = $pkg->new('sessions',store => 'db', bag => 'sessions', limit => 2, start => 2)->fix({ sessions => '' });
    my $expected = {
        sessions => {
            start => 2,
            limit => 2,
