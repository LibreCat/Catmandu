...
my $fixer = Catmandu::Fix->new(fixes => ['upcase("list.2.fav.color")']);
...
my $fixer = Catmandu::Fix->new(fixes => ['test.fix']);
my $importer = Catmandu::Importer::YAML->new(file => 'test.yaml');
