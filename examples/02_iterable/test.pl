...
my $importer = Catmandu::Importer::Mock->new(size => 10);

# To return all items in the importer as an array we use the to_array method
my $ref =  $importer->to_array;
...
print "[slice]\n";
print Dumper($importer->slice(3,2)->to_array);
...
print "[any]\n";
my $answer = $importer->any(sub { shift->{n} > 4});
...
print "[many]\n";
my $answer = $importer->many(sub { shift->{n} > 8});
...
# Mock are numeric
print "[all]\n";
my $answer = $importer->all(sub { shift->{n} =~ /^\d+$/});
...
# sum.
print "[reduce]\n";
my $result = $importer->reduce(0,sub {
     my $prev = shift;
     my $this = shift->{n} * 2;
     $prev + $this;
});
...
print "[select]\n";
print Dumper($importer->select(sub { shift->{n} > 5})->to_array);
...
# where n is not > 5.
print "[reject]\n";
print Dumper($importer->reject(sub { shift->{n} > 5})->to_array);
...
# iterator.
print "[pluck]\n";
print Dumper($importer->pluck('n')->to_array);
...
print "[includes]\n";
my $result = $importer->includes({ n => 42 });
...
print "[group]\n";
my $importer = Catmandu::Importer::Mock->new(size => 100);
my $groupnr  = 1;
$importer->group(10)->each(sub {
    my $it = shift;
